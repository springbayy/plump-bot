...
run_dir="${PLUMP_V7_RUN_DIR:-$repo_dir/checkpoints/v7_8m_expert_seed1}"
warm_start="${PLUMP_V7_WARM_START:-$repo_dir/checkpoints/v6_8m_fastppo_seed1/best.pt}"
...
if (( ${#checkpoints} > 0 )); then
  ...
  print -r -- "Resuming v7 expert iteration from $newest" >> "$run_dir/train.log"
else
  args+=(--initialize-from-v4 "$warm_start")
  print -r -- "Warm-starting v7 expert iteration from v4 checkpoint $warm_start (fresh Q heads)" \
    >> "$run_dir/train.log"
fi
